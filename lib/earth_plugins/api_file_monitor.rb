...
class ApiFileMonitor
  ...
  def update(directories, update_time = 0, *args)
    ...
    eta_printer = ApiEtaPrinter.new(self, "Updating directories", remaining_count) if options[:show_eta]
    start = Time.new
  ...
  def initial_pass_on_new_directory(name, parent = nil)
    ...
    benchmark "Scanning and storing tree for #{name}" do
      ...
      benchmark "Committing initial directory structure for #{name} to database" do
        @directory_eta_printer = ApiEtaPrinter.new(self, "Committing initial directory structure for #{name} to database", directory_count) unless parent
      ...
      benchmark "Creating cache information for #{name}" do
        ...
        @cached_size_eta_printer = ApiEtaPrinter.new(self, "Creating cache information for #{name}", directory_count) unless parent
